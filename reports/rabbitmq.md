...
| OpenSSL (TLS, crypto NIF) | full (SIMD, assembly) | full | partial (C scalar + vector extension support in 3.x) | RabbitMQ uses OpenSSL via Erlang `crypto` application. See the OpenSSL RISC-V status report for detail. |
