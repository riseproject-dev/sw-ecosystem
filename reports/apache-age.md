...
| lib/pq (`github.com/lib/pq`) | Postgres driver dependency for Go driver | Green | Green | Green | No riscv64 issues found. Pure Go, no cgo - rides on Go's riscv64 port, see `reports/go.md` |
...
| pgJDBC (`org.postgresql:postgresql`) | JDBC driver dependency | Green | Green | Green | No riscv64 issues found. Pure Java; runtime is the JVM, see `reports/openjdk.md` (JDBC driver requires Java 17 per `build.gradle.kts`) |
...
**Cross-references to existing scope.yml reports:** PostgreSQL to `reports/postgresql.md`; glibc to `reports/glibc.md`; Go (driver toolchain) to `reports/go.md`; Python (driver toolchain) to `reports/python.md`; OpenJDK (JDBC driver runtime) to `reports/openjdk.md`; PostGIS - tracked in `scope.yml` (`reports/postgis.md`) but confirmed not an AGE dependency.
