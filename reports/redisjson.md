...
|---|---|---|---|---|---|
| Redis | Host process; loads librejson.so | Builds (scalar C); no riscv64 CI | No upstream CI; community-tested | No official riscv64 binaries | See reports/redis.md |
...
| zstd (via zstd-rs 0.13) | Compression inside ijson for large JSON values | Supported; RVV vectorization merged (upstream PR #4435, 2025-07) | Yes, QEMU riscv64 CI in upstream | Not applicable (compiled into .so) | See reports/zstd.md |
...
| RediSearch | Runtime co-dependency; default Dockerfile loads redisearch.so | Not supported; CI is x86_64 and aarch64 only | No | No riscv64 release | See reports/redisearch.md |
...
The co-deployment dependency on RediSearch is tracked separately in `reports/redisearch.md`.
