...
3. Connect to a PostgreSQL server (which must run separately; see `reports/postgresql.md` for server-side riscv64 status)
...
|---|---|---|---|---|---|---|
| OpenJDK | 8+ (21 recommended) | JVM runtime | Yes (JEP 422, JDK 19+) | Full jtreg pass | Temurin/Corretto/Zulu all ship riscv64 | None; see reports/openjdk.md |
| PostgreSQL server | 9.1-18+HEAD | Database backend | Yes (generic C paths) | Regression tests pass | Packaged by Debian/Ubuntu/Fedora | No correctness blockers; see reports/postgresql.md |
...
- [OpenJDK RISC-V status report](reports/openjdk.md)
- [PostgreSQL server RISC-V status report](reports/postgresql.md)