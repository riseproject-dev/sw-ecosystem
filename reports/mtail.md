...
**Known build failures on riscv64:** None recorded in the repository. Given pure Go with no CGO, build failures would only arise from a Go toolchain bug. The Go toolchain has 209 open riscv64 issues (see [go.md](../reports/go.md)), but no mtail-specific build failure is known.
...
The Go toolchain is the only dependency with significant open riscv64 issues. See the Go report at [reports/go.md](../reports/go.md) for full detail. The issues relevant to mtail:
