...
| 2026-03-26 | (Dependency-layer, not `transformers` itself) `tokenizers` and `safetensors` merge riscv64 into their CI wheel matrices and begin shipping riscv64 wheels on PyPI | See [project-reports/tokenizers.md](project-reports/tokenizers.md), [project-reports/safetensors.md](project-reports/safetensors.md) |
...
|---|---|---|---|---|---|---|
| tokenizers (Rust) | Hard; fast tokenizer backend | Working - riscv64 added to CI wheel matrix, merged 2026-03-26 | Passing in CI | Shipping: `tokenizers-0.23.1-...manylinux_2_31_riscv64.whl` on PyPI | None open; earlier compile failure ([#1816](https://github.com/huggingface/tokenizers/issues/1816)) resolved via workaround before the fix landed | [project-reports/tokenizers.md](project-reports/tokenizers.md) |
| safetensors (Rust) | Hard; tensor serialization | Working - riscv64 added to CI wheel matrix, merged 2026-03-26 | Passing in CI | Shipping: `safetensors-0.8.0-...manylinux_2_31_riscv64.whl` on PyPI | None open | [project-reports/safetensors.md](project-reports/safetensors.md) |
| numpy (C, SIMD) | Hard; array/numeric backend | Working - wheel-build workflow merged 2026-05-27; promoted to NEP-57 Tier 3 (~Aug 2026) | Passing in CI on self-hosted RISE riscv64 runners | Not yet shipping on PyPI (latest 2.5.2 has no riscv64 wheel) despite green CI; community wheel index at gounthar.github.io/riscv64-python-wheels | Tier-3-to-official-wheel transition pending | [project-reports/numpy.md](project-reports/numpy.md) |
| torch (C++/CUDA) | Optional but de facto required for most model usage | Early stage - active tracking issue for RISC-V enablement, Phase 1 CI/build incomplete; XNNPACK RVV path reported broken [NEEDS VERIFICATION, single source] | No published pass-rate | Not shipping (latest 2.13.0, no riscv64 wheel) | uKernel/GEMM RVV kernels not implemented; XNNPACK cpuinfo gap | [project-reports/pytorch.md](project-reports/pytorch.md) |
| sentencepiece (C++) | Subword tokenizer used by many model configs | Working after toolchain fix (gcc bumped to gcc-14 to fix an abseil `static_assert` failure); briefly reverted for stability, now green | Passing (informational cross-build) | Not shipping on PyPI (latest 0.2.2); wheels only via community RISE index | Open issue asking whether official PyPI distribution will resume | [project-reports/sentencepiece.md](project-reports/sentencepiece.md) |
| tiktoken (Rust) | BPE tokenizer for some model configs | Proposed, not merged - CI wheel-build PR still open | Not run in CI | Not shipping (latest 0.13.0) | Open issue requesting riscv64 wheel, unresolved | [project-reports/tiktoken.md](project-reports/tiktoken.md) |
| faiss-cpu (C++/SIMD) | Vector similarity search for retrieval pipelines | Partial - CMake detects riscv64 and selects an RVV source set; enabling PR closed, not merged | Cross-compile SIMD smoke test only | Not shipping (latest 1.15.0); Debian sid has a native pre-RVV build | Prior "device build failed" issue closed without full fix; most RVV kernels remain scalar fallback | [project-reports/faiss.md](project-reports/faiss.md) |
| protobuf (C++) | Serialization used by sentencepiece and model config formats | Rejected - maintainers state "RISC-V isn't on our roadmap"; multiple riscv64 issues/PRs closed without merge | Not tested | Not shipping | Explicit maintainer non-commitment, no CI coverage | [project-reports/protocol-buffers.md](project-reports/protocol-buffers.md) |
| kenlm (C++) | Audio/CTC scoring extra | Unknown/unverified - no riscv64 issues found on a single GitHub search pass; not in project-reports/scope.yml | Unverified | No riscv64 wheel found on PyPI (version 0.3.0) | Not investigated further | Not in project-reports/scope.yml |
| huggingface-hub (pure Python) | Hard; model hub client | Pure Python, no native build concerns | N/A | Ships automatically (universal wheel) | None found | Not in project-reports/scope.yml |
...
- `torch`: this is the real gating item, and is tracked separately (see [project-reports/pytorch.md](project-reports/pytorch.md)); do not resize here, as it is a large, independently-scoped effort already covered by that report.

### 13.2 Performance Optimization

Not applicable to `transformers` itself (no native kernels exist to optimize). Any performance work is entirely a function of the `torch` RVV kernel roadmap; see [project-reports/pytorch.md](project-reports/pytorch.md). Do not size RVV kernel work under this report.
...
| Functional | `torch` riscv64 GA enablement (Phase 1 CI/build, XNNPACK RVV fixes) | Out of scope for this report - see [project-reports/pytorch.md](project-reports/pytorch.md) for sizing | RISE / PyTorch upstream | Critical (blocks real-world `transformers` usage) |
...
- [project-reports/tokenizers.md - Tokenizers riscv64 status report](project-reports/tokenizers.md)
- [project-reports/safetensors.md - Safetensors riscv64 status report](project-reports/safetensors.md)
- [project-reports/numpy.md - NumPy riscv64 status report](project-reports/numpy.md)
- [project-reports/pytorch.md - PyTorch riscv64 status report](project-reports/pytorch.md)
- [project-reports/sentencepiece.md - SentencePiece riscv64 status report](project-reports/sentencepiece.md)
- [project-reports/tiktoken.md - tiktoken riscv64 status report](project-reports/tiktoken.md)
- [project-reports/faiss.md - Faiss riscv64 status report](project-reports/faiss.md)
- [project-reports/protocol-buffers.md - Protobuf riscv64 status report](project-reports/protocol-buffers.md)