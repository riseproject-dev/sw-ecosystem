...
- `make check` uses `|| true` so CI jobs complete even with test failures; results are published to [libffi.github.io](https://libffi.github.io/libffi/project-reports/)
...
| CPython (consumer, not dep) | Uses libffi via `ctypes` | See `project-reports/python.md` | CPython carries `CTYPES_PASS_BY_REF_HACK` workaround for issue #694 | N/A | Issue #694 (libffi upstream bug) |
