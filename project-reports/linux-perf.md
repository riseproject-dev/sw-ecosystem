...
| OpenSSL | Required for `BUILD_BPF_SKEL`; perf kvm key validation | Builds | Debian sid, Ubuntu 24.04, Arch riscv64 all package OpenSSL 3.x; some distros lag vs. upstream | Version lag on some riscv64 distros; see `project-reports/openssl.md` (not yet written) |
...
| glibc | C runtime; `perf_event_open` ABI | Partial | Available in all major riscv64 distros | Known test failures in glibc test suite on riscv64; see `project-reports/glibc.md` (not yet written) |
...
| clang | Required for `BUILD_BPF_SKEL` | Builds | Available in all major riscv64 distros | LLVM riscv64 backend is Tier-1; see `project-reports/lldb.md` |
