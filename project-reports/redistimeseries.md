...
{% include dependency-graph.html slug="dependencies" subset="redistimeseries" %}
...
|---|---|---|---|---|---|
| [google/cpu_features](https://github.com/google/cpu_features) v0.6.0 (pinned) | Runtime CPU feature detection | Blocked - CMakeLists.txt emits FATAL_ERROR for unsupported arches including riscv64 | None | None | v0.10.1+ adds riscv; pinned copy is too old. See `project-reports/cpu_features.md`. |
...
| [libevent/libevent](https://github.com/libevent/libevent) | Async event loop (used by LibMR) | Builds - pure C, no arch restrictions | No riscv64 CI | No riscv64 binary | Architecture-agnostic. See `project-reports/libevent.md`. |
