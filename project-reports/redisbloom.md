...
{% include dependency-graph.html slug="dependencies" subset="redisbloom" %}
...
4. Run against a Redis server that also supports riscv64 (see `project-reports/redis.md` for Redis riscv64 status).
...
|------------|------|--------------|-------------|----------------|---------|
| Redis server | Required runtime host; RedisBloom loads as a `.so` module | Builds (no hard arch gate in Redis source) | No riscv64 CI in Redis repo | No riscv64 binary release | See `project-reports/redis.md` |
...
**Correct upstream target is Redis 8.** Since Redis 8 inlines all of these data structures, riscv64 enablement belongs in `redis/redis`, not in `RedisBloom/RedisBloom`. See `project-reports/redis.md` for the Redis riscv64 status. This report's findings for the standalone module are strategically secondary.
