...
| MySQL Server | Database backend managed by operator | builds from source (Debian sid 9.7.2-1) | limited | Debian only | No Oracle riscv64 binary. See [project-reports/mysql.md]. |
...
The operator itself (Python controller loop) has no architecture-specific performance paths. MySQL Server performance on riscv64 vs arm64 is covered separately in `project-reports/mysql.md`.
